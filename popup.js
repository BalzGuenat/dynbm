...
(async () => {
  ...
  port.onMessage.addListener(msg => {
    if (msg.action === "elementDescriptions") {
      renderElementDescriptions(port, msg.descriptions, targetElementId);
    } else if (msg.action === "showSelector") {
      showSelector(msg.selector);
    }
  });
...
function renderElementDescriptions(port, descriptions, targetElementId) {
  let outputStatus = document.getElementById("outputStatus")
